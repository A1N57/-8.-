...
def standart_writer(file_name2, res):
    with open(file_name2, 'w', encoding='utf-8', newline='') as data:
            f_w = DictWriter(data, fieldnames=['first_name', 'second_name', 'phone_number'])
            f_w.writeheader()
            f_w.writerows(res)

def copy_data(search_file, dest_file):
    search = int(input("Введите номер строки для копирования: "))
    search_res = read_file(search_file)
    if search <= len(search_res):
        entry_to_copy = search_res[search - 1]
        dest_res = read_file(dest_file)
        dest_res.append(entry_to_copy)
        standart_writer(dest_file, dest_res)
    else: 
        print("Введен неверный номер строки: ")
